...
var CORE_REQUIRED_FIELDS = ["atomic-task", "title", "created_date", "status"];
var VALID_STATUS_VALUES = ["todo", "in_progress", "blocked", "done"];
var VALID_PRIORITY_VALUES = ["low", "medium", "high"];
var CORE_SCHEMA_FIELDS = [
  {
    name: "atomic-task",
    type: "boolean",
    required: true,
...
function getCombinedSchemaFields(customFields) {
...
// src/settings/settings.tab.ts
var TaskSystemSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Enable schema validation").setDesc("Validate frontmatter schema for atomic task notes").addToggle(
    ...
    new import_obsidian.Setting(containerEl).setName("Auto-populate missing fields").setDesc("Automatically fill in required fields with default values when missing").addToggle(
    ...
    new import_obsidian.Setting(containerEl).setName("Validation mode").setDesc("How strict validation should be").addDropdown(
    ...
    new import_obsidian.Setting(containerEl).setName("Default task status").setDesc("Default status for new atomic tasks").addDropdown((dropdown) => {
      const options = {};
    ...
    new import_obsidian.Setting(containerEl).setName("Default task priority").setDesc("Default priority for new atomic tasks").addDropdown((dropdown) => {
      const options = {};
    ...
    new import_obsidian.Setting(containerEl).setName("Date format").setDesc("Format for date fields (currently only YYYY-MM-DD supported)").addText(
    ...
    new import_obsidian.Setting(containerEl).setName("Show success notifications").setDesc("Show notifications when validation passes without issues").addToggle(
    ...
    new import_obsidian.Setting(containerEl).setName("Validation delay (ms)").setDesc("Delay before validating files after changes (prevents excessive validation)").addSlider(
    ...
    new import_obsidian.Setting(containerEl).setName("Add new schema field").setDesc("Create a new custom field for atomic tasks").addButton(
...
var CustomFieldModal = class extends import_obsidian.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian.Setting(contentEl).setName("Display Name").setDesc("Human-readable name shown in UI").addText(
    ...
    let keyInput;
    new import_obsidian.Setting(contentEl).setName("Frontmatter Key").setDesc("The key used in frontmatter (lowercase, underscores allowed)").addText((text) => {
      keyInput = text;
    ...
    new import_obsidian.Setting(contentEl).setName("Field Type").setDesc("Data type for this field").addDropdown(
      (dropdown) => dropdown.addOptions({
        "text": "Text",
        "number": "Number",
        "date": "Date",
    ...
    new import_obsidian.Setting(contentEl).setName("Required").setDesc("Whether this field is required for all atomic tasks").addToggle(
      (toggle) => toggle.setValue(this.field.required).onChange((value) => {
        this.field.required = value;
      })
    );
    new import_obsidian.Setting(contentEl).setName("Default Value").setDesc("Default value when auto-populating (optional)").addText(
    ...
    this.updateEnumValuesVisibility();
    new import_obsidian.Setting(contentEl).setName("Description").setDesc("Optional description of this field").addTextArea(
...
var SchemaValidator = class {
   ...
   */
  static validate(frontmatter, customFields = []) {
    const result = {
      isValid: true,
      errors: [],
      warnings: []
  ...
  static validateLogicalRules(frontmatter, result) {
...
var NotificationManager = class {
  static showValidationWarnings(filename, result) {
    if (result.errors.length > 0) {
      ...
      result.errors.slice(0, 3).forEach((error, index) => {
        setTimeout(() => {
          new import_obsidian2.Notice(`Error: ${error.field} - ${error.message}`, 6e3);
        }, index * 1e3);
      });
    }
    if (result.warnings.length > 0) {
      ...
      if (result.warnings[0]) {
        setTimeout(() => {
          new import_obsidian2.Notice(`Warning: ${result.warnings[0].field} - ${result.warnings[0].message}`, 4e3);
        }, 500);
      }
    }
  }
  static showSuccessMessage(filename) {
    new import_obsidian2.Notice(`Schema validation passed for "${filename}"`, 2e3);
  }
  ...
  static showErrorMessage(message, duration = 5e3) {
    new import_obsidian2.Notice(`Error: ${message}`, duration);
  }
  ...
  static createDetailedNotice(title, details, duration = 8e3) {
    new import_obsidian2.Notice(title, duration);
    details.slice(0, 3).forEach((detail, index) => {
      setTimeout(() => {
        new import_obsidian2.Notice(detail, Math.max(3e3, duration - 2e3));
      }, (index + 1) * 800);
    });
  }
};
  ...
  static registerCommands(plugin) {
    ...
    plugin.addCommand({
      ...
      callback: async () => {
        ...
        new import_obsidian3.Notice(`Schema validation ${status}`);
      }
    });
    plugin.addCommand({
      ...
      callback: async () => {
        ...
        new import_obsidian3.Notice(`Auto-populate defaults ${status}`);
      }
    });
  }
};
...
// src/main.ts
var TaskSystemPlugin = class extends import_obsidian4.Plugin {
  async onload() {
